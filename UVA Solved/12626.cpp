...
int main()
{
    int t; map<char,int>piz;
    string s;
    cin>>t;
    while(t--)
    {
        cin>>s;
        int l=s.size(),m,a,r,g,i,t,num=123456;
        loop(i,0,l)
        {
            piz[s[i]]++;
        }
        {
            m=piz['M'];num=min(m,num);
            a=piz['A']/3;num=min(a,num);
            r=piz['R']/2;num=min(r,num);
            g=piz['G'];num=min(g,num);
            i=piz['I'];num=min(i,num);
            t=piz['T'];num=min(t,num);
        }
        cout<<num<<endl;
        piz.clear();
