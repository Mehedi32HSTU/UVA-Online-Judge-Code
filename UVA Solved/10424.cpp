...
int count_value(string s,int l)
{
    int a=0;
    loop(i,0,l)
    {
        if(s[i]>='a'&&s[i]<='z') a+=(s[i]-96);
        else if(s[i]>='A'&&s[i]<='Z') a+=(s[i]-64);
    }
    return a;
}
int value(int x)
...
int main()
{
    ...
    while(getline(cin,s1))
    {
        getline(cin,s2);
        int l=s1.size();
        int value1=count_value(s1,l);
        l=s2.size();
        int value2=count_value(s2,l);
        v1=value(value1);
        v2=value(value2);
        float s;
        if(v1>v2)
        {
            s=(float)v2*100/(float)v1;
        }
        else
        {
            s=(float)v1*100/(float)v2;
        }
        pf("%.2f %%\n",s);
    }
    return 0;
